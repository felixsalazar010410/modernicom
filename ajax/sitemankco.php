...
{	
require_once "../modelos/Sitemankco.php";

$sitemankco=new Sitemankco();
...
switch ($_GET["op"]){
	case 'guardaryeditar':
		...
		if (empty($idsitewom)){
			$rspta=$sitemankco->insertar($codigo,$nombre,$regional,$torrero,$especialista,$auditor,$imagen,$imagen2,$imagen3,$imagen4);
			echo $rspta ? "Sitio registrado" : "Sitio no se pudo registrar";
		}
		else {
			$rspta=$sitemankco->editar($idsitewom,$codigo,$nombre,$regional,$torrero,$especialista,$auditor,$imagen,$imagen2,$imagen3,$imagen4);
	...
	case 'desactivar':
		$rspta=$sitemankco->desactivar($idsitewom);
 		echo $rspta ? "Sitio Desactivado" : "Sitio no se puede desactivar";
	break;

	case 'activar':
		$rspta=$sitemankco->activar($idsitewom);
 		echo $rspta ? "Sitio activado" : "Sitio no se puede activar";
	break;

	case 'mostrar':
		$rspta=$sitemankco->mostrar($idsitewom);
 		//Codificar el resultado utilizando json
 		echo json_encode($rspta);
	break;

	case 'listar':
		$rspta=$sitemankco->listar();

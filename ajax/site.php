...
if (!isset($_SESSION["nombre"])) {
	...
	if ($_SESSION['proyecto_asecones'] == 1  && $_SESSION['asecones_escuelas_coordinador'] == 1 or $_SESSION['cliente_ofg'] == 1 or $_SESSION['cliente_ultratel'] == 1) {
		...
		switch ($_GET["op"]) {
			...
			case 'listar700documentadorsyf':

				if ($_SESSION['asecones_escuelas_coordinador'] == 1) {

					$rspta = $site->listar700syf();
					//Vamos a declarar un array
					$data = array();
				...
				case 'listar700documentadorsyfDominion':

					if ($_SESSION['asecones_escuelas_coordinador'] == 1) {
	
						$rspta = $site->listar700syfDominion();
						//Vamos a declarar un array
						$data = array();
						...
						while ($reg = $rspta->fetch_object()) {
							$data[] = array(
								"0" => ($reg->condicion) ? '<button class="btn btn-warning" onclick="mostrar(' . $reg->idsite . ')"><i class="fa fa-pencil"></i></button>' .
									...
									' <button class="btn btn-primary" onclick="activar(' . $reg->idsite . ')"><i class="fa fa-check"></i></button>',
								"1" => $reg->nombre,
								"2" => $reg->observaciones_doc,
								"3" => "<a href ='../files/tss_700/" . $reg->imagen . "' >$reg->imagen</a>",
								"4" => "<a href ='../files/packinglist_700/" . $reg->imagen2 . "'target='_blank'>$reg->imagen2</a>",
								"5" => "<a href ='../files/hwcontrol_700/" . $reg->imagen3 . "'target='_blank'>$reg->imagen3</a>",
								"6" => "<a href ='../files/seriales_700/" . $reg->imagen4 . "' >$reg->imagen4</a>",
								"7" => "<a href ='../files/comisionamiento_pre_700/" . $reg->imagen5 . "' >$reg->imagen5</a>",
								"8" => "<a href ='../files/reporte_radiante_pre_700/" . $reg->imagen6 . "' >$reg->imagen6</a>",
								"9" => "<a href ='../files/acta_hfd_700/" . $reg->imagen7 . "' >$reg->imagen7</a>",
								"10" => "<a href ='../files/acta_hfni_700/" . $reg->imagen8 . "' >$reg->imagen8</a>",
								"11" => "<a href ='../files/acta_hb_700/" . $reg->imagen9 . "' >$reg->imagen9</a>",
								"12" => "<a href ='../files/reporte_radiante_post_700/" . $reg->imagen10 . "' >$reg->imagen10</a>",
								"13" => "<a href ='../files/comisionamiento_post_700/" . $reg->imagen11 . "' >$reg->imagen11</a>",
								"14" => "<a href ='../files/registro_bts_700/" . $reg->imagen12 . "' >$reg->imagen12</a>",
								"15" => "<a href ='../files/pruebasvozydatos_700/" . $reg->imagen13 . "' >$reg->imagen13</a>",
								"16" => ($reg->condicion) ? '<span class="label bg-green">Aprobado</span>' :
									'<span class="label bg-yellow">En Proceso</span>'
							);
						}
			...
			case "selectProyecto":
				...
				while ($reg = $rspta->fetch_object()) {
					echo '<option value=' . $reg->idproyecto . '>' . $reg->operador .'-'. $reg->nombre . '</option>';
				}
				break;

				case "selectProyectosyf":
					require_once "../modelos/Proyecto.php";
					$proyecto = new Proyecto();
	
					$rspta = $proyecto->selectsyf();
					...
					case "selectProyectosyfDominion":
						require_once "../modelos/Proyecto.php";
						$proyecto = new Proyecto();
		
						$rspta = $proyecto->selectsyfDominion();
